...
public static lonlat2xyz( coord ){
...
public static quaternion(v0, v1) {
	...
	if (v0 && v1) {
        ...
        var theta = .5 * Math.acos(Math.max(-1, Math.min(1, DHelpers.dot(v0, v1)))),
...
// https://en.wikipedia.org/wiki/Rotation_formalisms_in_three_dimensions#Euler_angles_.E2.86.94_Quaternion
public static euler2quat(e) {

	if(!e) return;
    
    var roll = .5 * e[0] * DHelpers.to_radians,
        pitch = .5 * e[1] * DHelpers.to_radians,
        yaw = .5 * e[2] * DHelpers.to_radians,
...
public static quatMultiply(q1, q2) {
	if(!q1 || !q2) return;
...
*/
 ...
 public static eulerAngles(v0, v1, o0) {
